                ...
                <main class="py-12">
                    <x-section.section>
                            <div class="py-0 md:py-4 w-full max-w-screen-2xl mx-auto">
                                <div class="flex flex-col justify-center items-center border bg-zinc-100/90 dark:bg-zinc-800/80 w-full h-52 lg:h-64 border-gray-200/20 dark:border-zinc-900 rounded-[14px] p-0 md:p-2  px-4 ">
                                ...
                                <div class="flex justify-center items-center bg-white w-4/5 lg:w-3/4 h-22 lg:h-28 mx-auto border-b-teal-200/30 border-gray-200/20 dark:border-zinc-950 dark:bg-black rounded-[14px] p-0 md:p-2 mb-2 px-4 -mt-12 md:-mt-20 shadow-sm">
                                    <div class="py-4 flex justify-between">
                                        <div class="flex justify-center items-center gap-2 block flex-wrap text-center">
                                            <div class="flex lg:flex-col gap-1 ">
                                                <h3 class="font-semibold text-xs text-on-surface-strong dark:text-on-surface-dark-strong">Author:</h3>
                                                <span class="text-xs">{{ config('app.name','Laravel')}}</span>
                                            </div>
                                            <div class="flex lg:flex-col gap-1 px-8">
                                                <h3 class="font-semibold text-xs text-on-surface-strong dark:text-on-surface-dark-strong">Penulis:
                                                    </h3>
                                                <span class="text-xs">{{ $data->user->name }}</span>
                                            </div>
                                            <div class="flex lg:flex-col gap-1">
                                                <h3 class="font-semibold text-xs text-on-surface-strong dark:text-on-surface-dark-strong">Dilihat:
                                                    </h3>
                                                <span class="text-xs font-normal text-indigo-600"> {{ $viewer ?? '0' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if ($url == 'pokja')
                            <div class="lg:max-w-screen w-full max-w-screen-2xl mx-auto">
                                <x-page.article>
                                    <x-slot name="img">
                                        @isset($data->image)
                                        <img src="{{ url('storage/' . $data->image) }}" class="object-cover w-full rounded-md transition duration-700 ease-out group-hover:scale-105" alt="a penguin robot talking with a human" />

                                        @else
                                        <img src="{{ $img[0]['data']['content'] ?? 'https://img.freepik.com/premium-vector/default-image-icon-vector-missing-picture-page-website-design-mobile-app-no-photo-available_87543-11093.jpg' }}" class="object-cover w-full rounded-md transition duration-700 ease-out group-hover:scale-105" alt="a penguin robot talking with a human" />
                                        @endisset
                                    </x-slot>
                                    ...
                                    <x-slot name="content">
                                        ...
                                        <x-widget.share>
                                            <x-slot name="title">
                                                <span class="text-xs text-on-surface-strong dark:text-on-surface-dark-strong">Bagikan</span>
                                            </x-slot>
                                            <x-slot name="content">
                            ...
                            <div class="relative isolate overflow-hidden bg-white dark:bg-transparent px-2 py-6 sm:py-6 lg:overflow-visible lg:px-0">
                                <div class="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:items-start lg:gap-y-10">
                                  ...
                                  <div class="-mt-12 -ml-12 p-12 lg:sticky lg:top-4 lg:col-start-3 lg:row-span-2 lg:row-start-1 lg:overflow-hidden">
                                      <x-tabs.tabs-button class="lg:sticky lg:top-4 lg:overflow-hidden">
                                          <x-slot name="action">
                                              ...
                                              x-bind:aria-selected="selectedTab === 'new'" 
                                                        ...
                                                        aria-controls="tabpanelNew">
                                                        @if($url === 'pokja') Terkait @else Terbaru Di @endif {{ $data->categori->name }}
                                                </button>
                                                <button x-on:click="selectedTab = 'popular'" x-bind:aria-selected="selectedTab === 'popular'" 
                                                    ...
                                                    class="h-min px-4 py-2 text-sm" type="button" role="tab" aria-controls="tabpanelpPpular" >Populer Di {{ $data->categori->name }}
                                                </button>
                                            </x-slot>
                                            <x-slot name="content">
                                                <div x-cloak x-show="selectedTab === 'new'" id="tabpanelNew" role="tabpanel" aria-label="new">
                                                @forelse ($top_in_list as $item)
                                                <div class="mb-2 w-full max-w-sm border-outline bg-white p-2 text-left dark:border-zinc-500/30 dark:bg-zinc-800/10 dark:hover:border-sky-300/50 rounded-lg border" >
                                                    <div class="flex items-start gap-2 text-on-surface-strong dark:text-on-surface-dark-strong">
                                                        @isset($item->image)
                                                        <img src="{{ url('storage/' . $item->image ?? 'Padang') }}" class="size-16  rounded-md object-cover  hover:scale-105" alt="User avatar"/>
                                                        @else
                                                        <img src="https://img.freepik.com/premium-vector/default-image-icon-vector-missing-picture-page-website-design-mobile-app-no-photo-available_87543-11093.jpg" class="size-16  rounded-md object-cover  hover:scale-105" alt="User avatar"/>
                                                        @endisset
                                                        <span class="text-xs font-bold">{{ $item->title }}
                                                            <p class="text-xs font-extralight hover:underline focus:underline focus:outline-hidden text-zinc-800/50 dark:text-zinc-200">{{ $item->created_at->locale('id')->diffForHumans() }}</p>
                                                            <a href="{{ route('post.detail',$item->slug) }}" class="text-sm font-extralight hover:underline focus:underline focus:outline-hidden text-zinc-500/80 dark:text-zinc-200">Baca</a>
                                                        </span>
                                                    </div>
                                                </div>
